...
class _AnimatedIconWidgetState extends State<AnimatedIconWidget>
  ...
  void initState() {
    ...
    _animation.addListener(() {// addListener는 애니메이션의 값이 변경될 때마다 호출되는 메서드
      if(_animation.status == AnimationStatus.completed) {// 애니메이션이 완료되면 무한반복 하기 위해 다시 시작
        _controller.reverse(); // reverse는 애니메이션을 뒤에서부터 시작하는 메서드. 이 메서드를 호출하면 애니메이션이 뒤에서부터 시작
      } else if(_animation.status == AnimationStatus.dismissed) {// 애니메이션이 처음부터 다시 시작되면
